...
class AttentiveCNN(nn.Module):
    def __init__(self, embed_size, hidden_size, prompt_size):
        ...
        # Add the learned prompt as a parameter
        self.prompt_size = prompt_size
        self.learned_prompt = nn.parameter.Parameter(torch.FloatTensor(self.prompt_size, hidden_size),
                                                     requires_grad=True)
    ...
    def init_weights(self):
        ...
        nn.init.normal_(self.learned_prompt, 0, 1 / self.prompt_size)
    ...
    def forward(self, images):
        ...
        # Repeat the learned prompt across the batch and concat with V
        # E_t = Repeat(p_t, b), V = [V; E_t]
        E = self.learned_prompt.repeat(V.size(0), 1, 1)
        V = torch.cat([V, E], 1)
...
class Decoder(nn.Module):
    def __init__(self, embed_size, vocab_size, hidden_size, word_size, random_range=0.5):
        ...
        self.learned_word = nn.parameter.Parameter(
            torch.FloatTensor(self.word_size, embed_size).uniform_(-self.random_range, self.random_range),
            requires_grad=True)

    def forward(self, V, v_g, captions, states=None, initialize_from_vocab=True):

        # Word Embedding
        embeddings = self.embed(captions)

        # E_hat_t = Repeat(v_t, b), e_t = [e_t; E_hat_t]
        if initialize_from_vocab:
            E_hat = self.learned_word.repeat(embeddings.size(0), 1, 1)
            embeddings = torch.cat([E_hat, embeddings], 1)
...
class Encoder2Decoder(nn.Module):
    def __init__(self, embed_size, vocab_size, hidden_size, prompt_size, word_size):
        super(Encoder2Decoder, self).__init__()
        self.word_size = word_size
        # Image CNN encoder and Adaptive Attention Decoder
        self.encoder = AttentiveCNN(embed_size, hidden_size, prompt_size)
        self.decoder = Decoder(embed_size, vocab_size, hidden_size, word_size)
    ...
    def sampler(self, images, max_len=20):
        ...
        predicted = scores[:, -1:, :].max(2)[1]  # argmax
        captions = predicted
        sampled_ids.append(captions)
        attention.append(atten_weights[:, -1:, :])
        Beta.append(beta[:, -1:, :])
        for i in range(max_len):
            initialize_from_vocab = False
            scores, states, atten_weights, beta = self.decoder(V, v_g, captions, states, initialize_from_vocab)
